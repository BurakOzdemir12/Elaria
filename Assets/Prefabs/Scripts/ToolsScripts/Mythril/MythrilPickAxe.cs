...
public class MythrilPickAxe : MonoBehaviour
{
    [SerializeField] private int minMythrilPickAxeDamage = 49;
    [SerializeField] private int maxMythrilPickAxeDamage = 60;
    ...
    public void AttackCoroutine()
    {
        ...
        if (Physics.Raycast(ray, out hit, 4.4f))
        {
            ...
            {
                // Hasarý uygula
                stoneOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            SteelOreHealth steelOreHealth = hit.collider.GetComponent<SteelOreHealth>();
            if (steelOreHealth != null)
            {
                // Hasarý uygula
                steelOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            OrichalcumOreHealth orichalcumOreHealth = hit.collider.GetComponent<OrichalcumOreHealth>();
            if (orichalcumOreHealth != null)
            {
                // Hasarý uygula
                orichalcumOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            MythrilOreHealth mythrilOreHealth = hit.collider.GetComponent<MythrilOreHealth>();
            if (mythrilOreHealth != null)
            {
                // Hasarý uygula
                mythrilOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            TitaniumOreHealth titaniumOreHealth = hit.collider.GetComponent<TitaniumOreHealth>();
            if (titaniumOreHealth != null)
            {
                // Hasarý uygula
                titaniumOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            GoldOreHealth goldOreHealth = hit.collider.GetComponent<GoldOreHealth>();
            if (goldOreHealth != null)
            {
                // Hasarý uygula
                goldOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            CoalOreHealth coalOreHealth = hit.collider.GetComponent<CoalOreHealth>();
            if (coalOreHealth != null)
            {
                // Hasarý uygula
                coalOreHealth.takeDamage(Random.Range(minMythrilPickAxeDamage, maxMythrilPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
        }
        // Saldýrýyý bitir
