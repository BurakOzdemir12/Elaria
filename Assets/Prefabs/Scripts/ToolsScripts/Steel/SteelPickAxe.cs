...
public class SteelPickAxe : MonoBehaviour
{
    [SerializeField] private int minSteelPickAxeDamage = 5;
    [SerializeField] private int maxSteelPickAxeDamage = 20;
    ...
    public void AttackCoroutine()
    {
        ...
        if (Physics.Raycast(ray, out hit, 4.4f))
        {
            ...
            {
                // Hasarý uygula
                stoneOreHealth.takeDamage(Random.Range(minSteelPickAxeDamage, maxSteelPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            SteelOreHealth steelOreHealth = hit.collider.GetComponent<SteelOreHealth>();
            if (steelOreHealth != null)
            {
                // Hasarý uygula
                steelOreHealth.takeDamage(Random.Range(minSteelPickAxeDamage, maxSteelPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            OrichalcumOreHealth orichalcumOreHealth = hit.collider.GetComponent<OrichalcumOreHealth>();
            if (orichalcumOreHealth != null)
            {
                // Hasarý uygula
                orichalcumOreHealth.takeDamage(Random.Range(minSteelPickAxeDamage, maxSteelPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
            CoalOreHealth coalOreHealth = hit.collider.GetComponent<CoalOreHealth>();
            if (coalOreHealth != null)
            {
                // Hasarý uygula
                coalOreHealth.takeDamage(Random.Range(minSteelPickAxeDamage, maxSteelPickAxeDamage), transform.root.gameObject, PickaxeTag);
            }
        }
        // Saldýrýyý bitir
