...
public class TitaniumAxe : MonoBehaviour
{
    [SerializeField] private int minTitaniumAxeDamage = 49;
    [SerializeField] private int maxTitaniumAxeDamage = 60;
    ...
    public void AttackCoroutine()
    {
        ...
        if (Physics.Raycast(ray, out hit, 4.4f))
        {
            ...
            {
                // Hasarý uygula
                treeHealth.takeDamage(Random.Range(minTitaniumAxeDamage, maxTitaniumAxeDamage), transform.root.gameObject, axeTag);
            }
            BirchTreeHealth birchtreeHealth = hit.collider.GetComponent<BirchTreeHealth>();
            if (birchtreeHealth != null)
            {
                // Hasarý uygula
                birchtreeHealth.takeDamage(Random.Range(minTitaniumAxeDamage, maxTitaniumAxeDamage), transform.root.gameObject, axeTag);
            }
            NormalTreeHealth normalTreeHealth= hit.collider.GetComponent<NormalTreeHealth>();
            if (normalTreeHealth != null)
            {
                // Hasarý uygula
                normalTreeHealth.takeDamage(Random.Range(minTitaniumAxeDamage, maxTitaniumAxeDamage), transform.root.gameObject, axeTag);
            }
        }
        // Saldýrýyý bitir
