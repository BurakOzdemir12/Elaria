...
{
    [SerializeField] private int minstoneAxeDamage = 5;
    [SerializeField] private int maxstoneAxeDamage = 20;
    ...
    public void AttackCoroutine()
    {
        ...
        if (Physics.Raycast(ray, out hit, 4.4f))
        {
            ...
            {
                // Hasarý uygula
                treeHealth.takeDamage(Random.Range(minstoneAxeDamage,maxstoneAxeDamage), transform.root.gameObject,axeTag);
            }
            BirchTreeHealth birchtreeHealth = hit.collider.GetComponent<BirchTreeHealth>();
            if (birchtreeHealth != null)
            {
                // Hasarý uygula
                birchtreeHealth.takeDamage(Random.Range(minstoneAxeDamage,maxstoneAxeDamage), transform.root.gameObject,axeTag);
            }
            NormalTreeHealth normalTreeHealth = hit.collider.GetComponent<NormalTreeHealth>();
            if (normalTreeHealth != null)
            {
                // Hasarý uygula
                normalTreeHealth.takeDamage(Random.Range(minstoneAxeDamage, maxstoneAxeDamage), transform.root.gameObject, axeTag);
            }
        }
        // Saldýrýyý bitir
